...
def _populate_tabs_for_notebook(window, notebook_id):
    ...
    for section in sections:
        ...
        # Debug
        try:
            print(f"[ui_tabs] add tab sid={section_id}, title='{section_title}', idx={index}")
        except Exception:
            pass
...
def _load_first_page_for_current_tab(window):
    ...
    if title_edit:
        if pages:
            ...
            title_edit.setText("")
    # Reflect in right tree selection if there is a page
    if pages:
...
def _build_right_tree_for_notebook(window, notebook_id):
    ...
    if right_view is not None:
        ...
        # Expand all sections by default for quicker navigation
...
def _refresh_left_tree_children(window, notebook_id: int, select_section_id: int = None):
    ...
    if not tree_widget:
        return
    # Clear children for all notebooks to avoid stale states after deletes
    try:
        for i in range(tree_widget.topLevelItemCount()):
